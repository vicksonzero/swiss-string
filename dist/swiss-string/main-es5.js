...
var StepsService = /** @class */ (function () {
        ...
        for (var stepIndex = 0; stepIndex < steps.length; stepIndex++) {
            ...
            switch (type) {
                case _Step__WEBPACK_IMPORTED_MODULE_5__["WidgetType"].VIEW:
                    {
                        var beforeContext = this.updateViewWidgetContext(stepIndex, stepID, columns, contextHolders, newContexts);
                        newContexts.push(beforeContext);
                    }
                    break;
                case _Step__WEBPACK_IMPORTED_MODULE_5__["WidgetType"].OPERATOR:
                    {
                        var beforeContext = this.updateOperatorWidgetContext(stepIndex, stepID, columns, contextHolders, newContexts);
                        newContexts.push(beforeContext);
                    }
                    break;
            }
    ...
    StepsService.prototype.updateViewWidgetContext = function (stepIndex, stepID, columns, contextHolders, newContexts) {
        ...
        columns.forEach(function (column) {
            ...
            if (contextHolder[name]) {
                ...
                beforeContext.keys.push({
                    name: name, fromStepIndex: lastSeenStepIndex,
    ...
    StepsService.prototype.updateOperatorWidgetContext = function (stepIndex, stepID, columns, contextHolders, newContexts) {
        // mutates contextHolder and newContexts in place
        var beforeContext = {
            beforeStepID: stepID,
            keys: [],
